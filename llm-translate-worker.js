...
function buildPrompt(ctx, text, terms) {
  ...
  const rules = [
    ...
    '존댓말(합니다체)로 통일.',
    // ★ 계약 협상 자리에서 상대를 "너희" 라고 부르면 곤란하다.
    '호칭: 你们·贵方 → "귀사" 또는 "그쪽". "너희" 로 옮기지 말 것. 我们 → "저희".',
    '조각이 문장 중간에서 끊기면 끊긴 채로 둔다. "말입니다" 같은 맺음말을 덧붙이지 말 것.',
    '없는 말을 지어내지 않는다.',
    // ★ 자막이므로 사용자에게 말을 걸면 안 된다.
    //   실측: 뭉개진 조각을 받자 "죄송하지만 오인식이 심해서…재녹음해 주시면" 이라는
    //   안내문을 냈다. 회의 자막에 이런 문장이 뜨면 곤란하다.
    '너는 자막기다. 사용자에게 말을 걸지 마라. 사과·설명·요청·되묻기 금지.',
    '알아들을 수 없는 부분이 있어도 들린 만큼만 옮긴다. 전혀 못 알아들으면 아무것도 출력하지 않는다.',
    '오인식된 지명·고유명사는 맥락에 맞게 바로잡는다.',
    // ★ 인식기가 계약 조항의 条(조)를 线(선)으로 잘못 듣는다.
    //   조항 번호가 "27번 라인" 으로 나오면 회의에서 바로 문제가 된다.
    //   이건 문맥으로만 판단할 수 있어서 여기 적는다.
    '계약 이야기 중의 "第X线" 은 "第X条"(제X조)를 잘못 들은 것이다. "제X조"로 옮긴다.',
